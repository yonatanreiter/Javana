...
public class ProgramGenerator extends CodeGenerator {
     ...
     */
    public void emitProgram(JavanaParser.ProgramContext ctx) {
        programId = null; //ctx.programHeader().programIdentifier().entry;
     ...
     * @param ctx the ProgramContext.
     */
    private void emitMainMethod(JavanaParser.ProgramContext ctx) {
        emitLine();
        emitComment("MAIN");
        ...
        emitLine();
        emitComment("statements go here");
     ...
     */
    public void emitRoutine(JavanaParser.FuncDefinitionContext ctx) {
        SymTableEntry routineId = null; //ctx.procedureHead() != null ? ctx.procedureHead().routineIdentifier().entry : ctx.functionHead().routineIdentifier().entry;
        ...
        // Emit code for the compound statement.
        //JavanaParser.CompoundStatementContext stmtCtx = (JavanaParser.CompoundStatementContext) routineId.getExecutable();

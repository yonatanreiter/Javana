...
public class Semantics extends PascalBaseVisitor<Object> {
    ...
    public Object visitEnumerationTypespec(PascalParser.EnumerationTypespecContext ctx) {
        ...
        for (PascalParser.EnumerationConstantContext constCtx : ctx.enumerationType().enumerationConstant()) {
            ...
            else if(constants.contains(constantId)){
    ...
    public Object visitSubrangeTypespec(PascalParser.SubrangeTypespecContext ctx) {
        ...
        if (((minType.getForm() != ENUMERATION)) || (minType == Predefined.realType) || (minType == Predefined.stringType)   ) {
            ...
            if(minType != Predefined.charType) {
                if (minType != maxType) {
        ...
        if(minType == Predefined.charType && ((Character) minObj > (Character) maxObj)){
            error.flag(INVALID_CONSTANT, maxCtx);
            maxObj = 'z';
        }
    ...
    public Object visitArrayTypespec(PascalParser.ArrayTypespecContext ctx) {
        ...
        String type = ctx.arrayType().stop.getText();
    ...
    public Object visitAssignmentStatement(PascalParser.AssignmentStatementContext ctx) {
        ...
        if(symTableStack.getLocalSymTable().lookup(lhs) == null && ((PascalParser.LhsContext)ctx.children.get(0)).start.equals(((PascalParser.LhsContext)ctx.children.get(0)).stop)){
            error.flag(UNDECLARED_IDENTIFIER, ctx.lhs());
        }


        Set<String> numberNames = Set.of("integer", "real");

        if(symTableStack.getLocalSymTable().lookup(lhs) != null && numberNames.contains(symTableStack.getLocalSymTable().lookup(lhs).getType().getIdentifier().getName())){

            //rhs is a variable or its not a single child
            if(symTableStack.getLocalSymTable().lookup(ctx.children.get(2).getText()) == null && ((PascalParser.RhsContext)ctx.children.get(2)).start.equals(((PascalParser.RhsContext)ctx.children.get(2)).stop)) {
                try {
                    Integer.parseInt(ctx.children.get(2).getText());
                } catch (NumberFormatException e) {

                    try {
                        Double.parseDouble(ctx.children.get(2).getText());
                    } catch (NumberFormatException b) {
                        error.flag(INCOMPATIBLE_ASSIGNMENT, (PascalParser.RhsContext) ctx.children.get(2));
                    }
    ...
    @Override
    public Object visitRhs(PascalParser.RhsContext ctx) {
    ...
    public Object visitIfStatement(PascalParser.IfStatementContext ctx) {
        ...
        if(exprType != Predefined.booleanType){
        ...
        for (PascalParser.CaseBranchContext branchCtx : branchListCtx.caseBranch()) {
            ...
            if (constListCtx != null) {
                ...
                for (PascalParser.CaseConstantContext caseConstCtx : constListCtx.caseConstant()) {
                    ...
                    if(constValue instanceof Float && exprType.getForm().toString().equals("scalar")){
                        error.flag(MISMATCHED_TYPE, constCtx);
                    }
    ...
    public Object visitWhileStatement(PascalParser.WhileStatementContext ctx) {
        ...
        if(exprCtx.type != Predefined.booleanType){
            error.flag(TYPE_MUST_BE_BOOLEAN, exprCtx);
        }
            ...
            if (modCtx.indexList() != null) {
                ...
                for (PascalParser.IndexContext indexCtx : indexListCtx.index()) {
                    if (type.getForm() == ARRAY) {
                        ...
                        visit(exprCtx);
                        try{
                            Integer.parseInt(exprCtx.children.get(0).getText());
                        }catch (NumberFormatException e){
                            error.flag(MISMATCHED_TYPE, indexCtx);
                        }
